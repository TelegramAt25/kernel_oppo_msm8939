...
#define TPD_DEVICE "synaptics-s3203"
...
static ssize_t cap_vk_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
      /* LEFT: search: CENTER: menu ,home:search 412, RIGHT: BACK */
				   ...
				   return sprintf(buf,
					  ...
					  "\n",118,1370,120,90,360,1370,100,80,600,1370,120,90);
		  	} else {
		 return sprintf(buf,
				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":%d:%d:%d:%d"
			":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":%d:%d:%d:%d"
...
static void tpd_down(struct synaptics_ts_data *ts, int id, int raw_x, int raw_y, int x, int y, int p) 
{
    if( ts && ts->input_dev ){
		...
		TPD_DEBUG("Synaptics:Down[%d %4d %4d %4d]\n", id, x, y, p);	
...
static void int_key_report_s3203(struct synaptics_ts_data *ts)
{
    int ret= 0;
...
/*disable irq when read data from IC*/	
	...
	ret = i2c_smbus_read_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8);
	tmp_old = ret&0xff;
	printk("ret = %x ,tmp_old =%x ,tmp_new = %x\n",ret,tmp_old,(tmp_old & 0xef));
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8,(tmp_old & 0xef));
	ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_COMMAND_BASE,0x04);
	checkCMD();
	TPD_DEBUG("forbid CBC oK\n");
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE + 81,0X01);
...
TEST_WITH_CBC_3203:
	...
	if(enable_cbc){
		...
		ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8,(tmp_old | 0x10));
	...
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CTRL41,0x01);
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0X04);//force F54_ANALOG_CMD00
    checkCMD();
	TPD_DEBUG("forbid Forbid NoiseMitigation oK\n");
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x02);//Force Cal, F54_ANALOG_CMD00
    checkCMD();
	...
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8,tmp_old); //restore CBC
	ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_COMMAND_BASE,0x04);		//force update
	checkCMD();
	ret = i2c_smbus_read_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8);
    ...
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_DATA_BASE,0x05);//select report type 0x05	 
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x01);//get report
    checkCMD();
    ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_DATA_BASE+1,0x0);	
    tx_datal = i2c_smbus_read_word_data(ts->client,F54_ANALOG_DATA_BASE+3);
    tx_datah = i2c_smbus_read_word_data(ts->client,F54_ANALOG_DATA_BASE+3);
	...
	//Reset before rx-rx test.
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x00);
    ret = i2c_smbus_write_byte_data(ts->client, F01_RMI_CMD00,0x01);
    msleep(150);
	ret = synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL00, 0x84); //Set to no sleep mode 
	ret = i2c_smbus_read_byte_data(ts->client, F01_RMI_DATA_BASE);
	
	
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x1); //Change to page 01	
	ret = i2c_smbus_read_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8);
	tmp_old = ret&0xff;
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8,(tmp_old & 0xef));	//Disable CBC
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x04);//force update
	checkCMD();
	
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CTRL41,0x01);//No SignalClarity,F54_ANALOG_CTRL41
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x04);//force update
	checkCMD();
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x02);//Force Cal,F54_ANALOG_CMD00
    checkCMD();
 ...
 //report type7 fills tx rows of data
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_DATA_BASE,7);//select report type 0x07
	
	ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0x01);//get report
    checkCMD();
    ret = i2c_smbus_read_byte_data(ts->client,F54_ANALOG_COMMAND_BASE);//read report
	TPDTM_DMESG("F54_ANALOG_CMD00[2]=%d \n",ret);

	ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_DATA_BASE+1,0);
	for(x = 0;x < TX_NUM; x++) {
		for(y = 0; y < RX_NUM; y++) {
			ret= i2c_smbus_read_word_data(ts->client,F54_ANALOG_DATA_BASE+3);
 ...
 //report type17 fills tx->rx rows of data
    ...
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_DATA_BASE,17);//select report type 0x17 
    ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_DATA_BASE+1,0);
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_COMMAND_BASE,0X01);//get report
    checkCMD();
...
END:
    ...
    ret = i2c_smbus_write_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8,tmp_old);
	ret = i2c_smbus_write_word_data(ts->client,F54_ANALOG_COMMAND_BASE,0x04);
	checkCMD();
	ret = i2c_smbus_read_byte_data(ts->client,F54_ANALOG_CONTROL_BASE+8);
	TPDTM_DMESG("[s3202]tem_new end = %x",ret&0xff);
...
static ssize_t synaptics_rmi4_baseline_show(struct device *dev,	char *buf, bool savefile)	
{
	return synaptics_rmi4_baseline_show_s3203(dev,buf,savefile);	
}
...
static int	synaptics_input_init(struct synaptics_ts_data *ts)
{
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		TPD_ERR("%s: Failed to allocate input device\n",__func__);
		return -1;
	}
...
static  void get_tp_id(int TP_ID1,int TP_ID2,int TP_ID3, struct synaptics_ts_data *ts)
{
	...
	/*huqiao@EXP.BasicDrv.Basic add for tp 15399*/
	if(is_project(OPPO_15399))
		{
			if((id1 == 0)&&(id2== 0)){
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if( (ts->boot_mode == MSM_BOOT_MODE__FACTORY || ts->boot_mode == MSM_BOOT_MODE__RF || ts->boot_mode == MSM_BOOT_MODE__WLAN) ){
		...
		if (!IS_ERR(ts->vdd_2v8)){
			ret = regulator_disable(ts->vdd_2v8);
			if (ret) {
...
/*huqiao@EXP.BasicDrv.Basic add for tp 15399*/
        	...
        	if(tp_dev == TP_OFILM){
        		...
        		strcpy(ts->fw_name,"tp/15399/15399_Firmware_Oflim.img");
        		strcpy(ts->test_limit_name,"tp/15399/15399_Limit_Oflim.img");
        	}else if(tp_dev == TP_BIEL){
        		sprintf(ts->manu_name, "TP_BIEL");
        		strcpy(ts->fw_name,"tp/15399/15399_Firmware_Biel.img");
        		strcpy(ts->test_limit_name,"tp/15399/15399_Limit_Biel.img");
        	} else {
        		sprintf(ts->manu_name, "TP_TRULY"); 
        		strcpy(ts->fw_name,"tp/15399/15399_Firmware_Truly.img");
        		strcpy(ts->test_limit_name,"tp/15399/15399_Limit_Truly.img");
        	}        
       }
#endif  
